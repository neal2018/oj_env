...
struct HeavyLight {
  ...
  vector<array<int, 2>> get_dfn_path(int x, int y) const {
    ...
    while (top[x] != top[y]) {
      ...
      path[front].push_back({dfn[top[y]], dfn[y]});
      y = parent[top[y]];
    }
    if (deep[x] > deep[y]) swap(x, y), front = !front;

    path[front].push_back({dfn[x], dfn[y]});
...
int main() {
  ...
  while (T--) {
    ...
    for (const auto &[u, v, k] : queries) {
      ...
      auto node_left = [&](int x) {
        Node res;
        while (hld.top[x] != hld.top[lca]) {
          auto addon = seg.query(hld.dfn[hld.top[x]], hld.dfn[x] + 1);
          res = pull(addon, res);
          x = hld.parent[hld.top[x]];
        }
        auto addon = seg.query(hld.dfn[lca], hld.dfn[x] + 1);
        res = pull(addon, res);
        return res;
      }(u);
      auto node_right = [&](int x) {
        Node res;
        while (hld.top[x] != hld.top[lca]) {
          auto addon = seg.query(hld.dfn[hld.top[x]], hld.dfn[x] + 1);
          res = pull(addon, res);
        ...
        if (x != lca) {
          auto addon = seg.query(hld.dfn[lca] + 1, hld.dfn[x] + 1);
          res = pull(addon, res);
        }
        return res;
      }(v);

      swap(node_right.max_pre_sum, node_right.max_suf_sum);
      swap(node_right.min_pre_sum, node_right.min_suf_sum);

      const auto node = pull(node_right, node_left);
