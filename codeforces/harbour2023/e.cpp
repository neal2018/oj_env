...
int main() {
  ...
  while (T--) {
    ...
    vector<int> a(n);
    for (auto &x : a) cin >> x;
    vector<vector<int>> tire{vector<int>{-1, -1}};
    vector<int> ends{0}, go{0};
    for (auto &x : a) {
      int p = 0;
      for (int bit = 30; bit >= 0; bit--) {
        int cur = (x >> bit) & 1;
        if (tire[p][cur] == -1) {
          tire[p][cur] = int(tire.size());
          tire.push_back({-1, -1});
          go.push_back(0), ends.push_back(0);
        }
        go[p]++;
        p = tire[p][cur];
      }
    ...
    for (auto &x : a) {
      ...
      for (int bit = 30; bit >= 0; bit--) {
        int cur = (x >> bit) & 1;
        if (cur == 0) {
          if (tire[p][1] != -1) res += go[tire[p][1]] * (bit_cnt + 1 + (bit_cnt & 1));
        } else {
          if (tire[p][0] != -1) res += go[tire[p][0]] * (bit_cnt + 1 + ((bit_cnt & 1) ^ 1));
          ++bit_cnt;
        }
        p = tire[p][cur];
      }
      res += (bit_cnt + 1) * ends[p];
    }
    cout << (res / n / n) << "\n";
  }
}
