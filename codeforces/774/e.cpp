...
int main() {
  ...
  int n, m;
  cin >> n >> m;
  vector<int> a(n), p(m), is_p(n);
  for (auto &x : a) cin >> x;
  for (auto &x : p) cin >> x, x--, is_p[x] = 1;

  vector<Z> p2(m + 1, 1);
  for (int i = 1; i <= m; i++) p2[i] = 2 * p2[i - 1];
  Z res = 0;
  for (auto rot : {0, 1}) {
    vector<Z> first(n), second(n);
    for (int i = 0; i < m; i++) {
      first[p[i]] = p2[i];
      if (p[i] - rot >= 0) second[p[i] - rot] = p2[m - 1 - i];
    }
    for (int i = n - 2; i >= 0; i--) second[i] = second[i] + second[i + 1];
    vector<Z> ways(2 * n);
    auto solve = [&](auto self, int l, int r, vector<Z> &c) {
      if (r - l == 1) return;
      int mid = (l + r) / 2;
      self(self, l, mid, c);
      self(self, mid, r, c);
      Poly aa(vector<Z>(first.begin() + l, first.begin() + mid));
      Poly bb(vector<Z>(second.begin() + mid, second.begin() + r));
      auto prod = aa * bb;
      for (int i = 0; i < prod.size(); i++) c[i + l + mid] += prod[i];
    };
    solve(solve, 0, n, ways);
    vector<Z> to_sub(2 * n);
    for (int i = 0; i < n; i++) first[i] *= i;
    solve(solve, 0, n, to_sub);
    for (int i = 0; i < n; i++) res += a[i] * (i * ways[2 * i] - to_sub[2 * i]);
    Z tot = 0;
    for (int i = 0, pref = 0; i < n; i++) {
      res += a[i] * (i * (p2[pref] - 1) - tot);
      if (is_p[i]) {
        tot += i * p2[pref];
        pref++;
      }
    ...
    reverse(is_p.begin(), is_p.end());
    reverse(p.begin(), p.end());
    for (auto &pos : p) pos = n - 1 - pos;
  }
  res /= (p2[m] - 1);
  cout << res << "\n";
}
