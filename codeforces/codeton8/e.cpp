...
int main() {
  ...
  while (T--) {
    int l, n;
    std::cin >> l >> n;
    int total_empty = l - 2 * n;
    Z res = 0;
    for (int inside = 0; inside <= total_empty; inside++) {
      int outside = total_empty - inside;
      if (inside % 2) {
        continue;
      }
      res += comb.binom(inside / 2 + n - 1, n - 1) * comb.binom(outside + n, n);
    }
    std::cout << 2 * (comb.binom(l, 2 * n) - res) << "\n";
  }
}
