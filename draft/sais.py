...
def buildTypeMap(data: List[int]):
    """
...
def showTypeMap(data):
    ...
    print("".join([chr(x) for x in typemap]))
    print("".join("^" if isLMSChar(i, typemap) else " " for i, _ in enumerate(typemap)))


x = makeSuffixArrayByInducedSorting([ord(x) for x in 'banana'], 256)
print(x)
