...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      Z not_in = comb.binom(i * m, k) + comb.binom((n - i - 1) * m, k) + comb.binom(n * j, k) +
                 comb.binom(n * (m - j - 1), k);
      not_in -= comb.binom(i * j, k) + comb.binom((n - i - 1) * j, k) +
                comb.binom(i * (m - j - 1), k) + comb.binom((n - i - 1) * (m - j - 1), k);
      res += not_in;
    }
  }
  res /= comb.binom(n * m, k);
  cout << (n * m - res) << "\n";
}
