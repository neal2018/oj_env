...
auto area2(const vector<Point> &p) {
  int n = (int)p.size();
  long long area = 0;
  for (int i = 0; i < n; i++) area += p[i] ^ p[(i + 1) % n];
  return area;
...
int main() {
  ...
  auto total2 = area2(points);
  ll res = 9e18, s = 0;
  for (ll i = 0, j = 0; i < n; i++) {
    while (s < total2) {
      s += 4 * area2(points[i], points[j], points[(j + 1) % n]);
      j = (j + 1) % n;
      res = min(res, abs(s - total2));
    }
    s -= 4 * area2(points[j], points[i], points[(i + 1) % n]);
    res = min(res, abs(s - total2));
  }
  cout << res << "\n";
}
