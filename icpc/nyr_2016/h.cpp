...
Point a = {0, 0}, b, c;
ld dab, dbc, dca;
ld get_dist(const Point &p) {
  bool lab = (((a - p) ^ (b - p)) >= eps);
  bool lbc = (((b - p) ^ (c - p)) >= eps);
  bool lca = (((c - p) ^ (a - p)) >= eps);
  if (lab && lbc && lca) return dab + dbc + dca;
  if (!lca && !lbc) {
    return dist(p, a) + dist(p, b) + dab;
  } else if (lca && lab) {
    return dca + dab + dist(p, c) + dist(p, b);
  } else if (!lab && !lbc) {
    return dist(p, a) + dist(p, c) + dca;
  } else if (lca && lbc) {
    return dca + dbc + dist(p, a) + dist(p, b);
  } else if (!lca && !lab) {
    return dist(p, b) + dist(p, c) + dbc;
  } else {
    return dab + dbc + dist(p, a) + dist(p, c);
  }
}
int main() {
  ...
  while (T--) {
    ...
    cin >> bx >> cx >> cy >> L;
    b = {bx, 0}, c = {cx, cy};
    dab = dist(a, b), dbc = dist(b, c), dca = dist(c, a);
    ld MAX = L / 2 + 1;
    auto f = [&](ld y) {
      ld lm = 0, rm = 0, cut = 0;
      if (y > 0) {
        ...
        for (int i = 0; i < 8; i++) {
          ...
          ld dis1 = get_dist({mid1, y});
          ld dis2 = get_dist({mid2, y});
          if (dis1 < dis2) {
            r = mid2;
          } else {
            l = mid1;
    ...
    ld res = adaptive_simpson(-MAX, MAX, f);
    cout << tt << " " << res << "\n";
  }
}