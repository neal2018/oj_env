...
int main() {
  ...
  for (auto &point : a) {
    int c;
    cin >> c >> point.x >> point.y;
    point.x = point.x * A / c, point.y = point.y * A / c;
  }
  ld res = a[0].x * a[0].y;
  for (auto &point : a) {
    res = max(res, point.x * point.y);
  }
  ...
  for (int i = 0; i < sz; i++) {
    ...
    while (r - l >= eps) {
      ...
      // cout << mid1 << " " << mid2 << "\n";
      ld p1 = (mid1 * ah + (1 - mid1) * bh) * (mid1 * ap + (1 - mid1) * bp);
      ld p2 = (mid2 * ah + (1 - mid2) * bh) * (mid2 * ap + (1 - mid2) * bp);
      // cout << p1 << " " << p2 << "\n";
      if (p1 > p2) {
        r = mid2;
      } else {
