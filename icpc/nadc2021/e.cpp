...
vector<Point> get_convex(vector<Point> &points) {  // strict, two pass, can collinear
  ...
  for (int i = U.size() - 1; i >= 1; i--) L.push_back(U[i]);
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
      if (!is_parallel(lines[i], lines[j])) {
        ps.push_back(intersect(lines[i], lines[j]));
      }
    }
  }
  ...
  int sz = ps.size();
  ll res = 0;
  auto check = [&](const vector<Point> &t) {
    int f = 1;
    set<int> seen;
    for (int xx = 0; xx < 4; xx++) {
      for (int ii = 0; ii < n; ii++) {
        if (on_line(lines[ii], t[xx]) && on_line(lines[ii], t[(xx + 1) % 4])) {
          seen.insert(ii);
          break;
        }
      }
    ...
    for (int j = i + 1; j < sz; j++) {
      for (int k = j + 1; k < sz; k++) {
        for (int l = k + 1; l < sz; l++) {
          ...
          Segment d{ps[j], ps[l]};
          if (!is_intersect(a, b)){
            check({ps[i], ps[k], ps[j], ps[l]});
          }
          if (!is_intersect(c, d)){
            check({ps[i], ps[j], ps[k], ps[l]});
          }
        }
      }
    }
