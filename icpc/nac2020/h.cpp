...
HashInt mod{1000000067ll};

HashInt operator*(const HashInt& a, const HashInt& b) {
  return {(a.v * b.v % mod.v + mod.v) % mod.v};
}

HashInt operator+(const HashInt& a, const HashInt& b) { return {(a.v + b.v) % mod.v}; }

HashInt operator-(const HashInt& a, const HashInt& b) { return {(a.v - b.v + 10 * mod.v) % mod.v}; }
HashInt operator*(const HashInt& a, ll b) {
  // cout<<a.first<<" "<<a.second<<"\n";
  // cout<<b<<"\n";
  // cout<<mod.first<<" "<<mod.second<<"\n\n";
  // cout<<((-1) %1000000069)<<"\n";
  // cout<<((-1) % mod.first + mod.first)<<"\n";
  return {((a.v * b) % mod.v + mod.v) % mod.v};
}
...
int main() {
  ...
  auto hash_a = get_hash(aa);
  auto hash_b = get_hash(bb);
  auto hash_c = get_hash(cc);
  ...
  ll res = inf;
  auto solve = [&](string& sa, string& sb, string& sc, HashInt a, HashInt b, HashInt c) {
    for (ll i = 0; i < n; i++) {
      auto temp_c = c;
      for (ll j = 0; j < n; j++) {
