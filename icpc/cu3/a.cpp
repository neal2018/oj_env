...
ld adaptive_simpson(ld _l, ld _r, function<ld(ld)> f) {
  ...
  function<ld(ld, ld, ld)> asr = [&](ld l, ld r, ld s) {
    ...
    if (abs(left + right - s) <= 1e-3) return left + right;
...
int main() {
  ...
  auto f = [&]() {
    ...
    r1 = r1 * PI / 180;
    r2 = r2 * PI / 180;
    auto get_circle = [&](const Point &A, const Point &B, ld degree) -> vector<Circle> {
      auto ab1 = B - A;
      auto mid = (B + A) / 2;
      ld b1 = dist(ab1) / 2 / (tan(degree / 2));
      Point c11 = mid + perp(ab1) / dist(ab1) * b1;
      Point c12 = mid - perp(ab1) / dist(ab1) * b1;
      auto c1 = get_circumscribed(A, B, c11);
      auto c2 = get_circumscribed(A, B, c12);
      return {c1, c2};
    };
    auto check = [&](const Point &X) {
      if (X == a || X == b || X == c) return false;
      Point ax = a - X;
      Point bx = b - X;
      Point cx = c - X;
      ld axb = (ax * bx) / dist(ax) / dist(bx);
      ld bxc = (bx * cx) / dist(bx) / dist(cx);
      // cout << "deg: " << axb << " " << bxc << endl;
      // cout << "deg: " << cos(r1) << " " << cos(r2) << endl;
      // cout << "deg: " << r1 << " " << r2 << endl;
      if (sgn(axb - cos(r1)) || sgn(bxc - cos(r2))) return false;
      return true;
    };
    auto can1 = get_circle(a, b, r1);
    auto can2 = get_circle(b, c, r2);
    for (auto &cc : can1) {
      for (auto &cc2 : can2) {
        auto t = intersect(cc, cc2);
        if (cc.o == cc2.o && sgn(cc.r - cc2.r) == 0) {
          for (auto &t : {a, b, c}) {
            auto x = cc.o + cc.o - t;
            if (check(x)) {
              cout << x.x << " " << x.y << "\n";
