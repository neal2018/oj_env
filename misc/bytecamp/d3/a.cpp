...
int main() {
  ...
  ld x1, y1, x2, y2;
  cin >> x1 >> y1 >> x2 >> y2;
  auto get_inter = [](ld p, ld x, ld y) -> vector<pair<ld, ld>> {
    Line l = {{0, p}, {1, p}};
    auto large_inter = intersect({{x, y}, 100}, l);
    if (large_inter.size() < 2) return {};
    if (p < y - 100 || p > y + 100) return {};
    if (p >= y + 60) {
      return {{large_inter[0].x, large_inter[1].x}};
    }
    auto inter1 = intersect({{x - 40, y + 30}, 30}, l);
    auto inter2 = intersect({{x + 40, y + 30}, 30}, l);
    if (p > y && inter1.size() > 1 && inter2.size() > 1) {
      return {{large_inter[0].x, inter1[0].x},
              {inter1[1].x, inter2[0].x},
              {inter2[1].x, large_inter[1].x}};
    }
    if (p >= y - 20) {
      return {{large_inter[0].x, large_inter[1].x}};
    }
    auto inter3 = intersect({{x, y - 20}, 60}, l);
    if (p > y - 80 && inter3.size() > 1) {
      return {{large_inter[0].x, inter3[0].x}, {inter3[1].x, large_inter[1].x}};
    }
    return {{large_inter[0].x, large_inter[1].x}};
  };
  auto f = [&](ld y) {
    vector<pair<ld, ld>> inter = get_inter(y, x1, y1);
    for (auto &x : get_inter(y, x2, y2)) inter.push_back(x);
    sort(inter.begin(), inter.end());
    if (inter.size() == 0) return ld(0);
    vector<pair<ld, ld>> nex = {inter[0]};
    for (int i = 1; i < inter.size(); i++) {
      if (inter[i].first <= nex.back().second) {
        nex.back().second = max(inter[i].second, nex.back().second);
      } else {
        nex.push_back(inter[i]);
      }
    }
    ld res = 0;
    for (auto &[a, b] : nex) res += b - a;
    return res;
  };
  if (abs(x1 - x2) > 200 || abs(y1 - y2) > 200) {
    cout << "40212.3859659493537037633\n";
    return 0;
  }
  auto t = adaptive_simpson(min(y1, y2) - 100, max(y1, y2) + 100, f);
  cout << t << "\n";
}
